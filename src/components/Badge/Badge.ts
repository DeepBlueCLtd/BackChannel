...
export function createBadge(options: BadgeOptions): HTMLElement {
  ...
  badgeContainer.className = `bc-badge-container bc-badge-${position}`
  ...
  badge.className = `badge badge-${options.enabled ? 'primary' : 'light'}`
  ...
  // Create the BC text/icon
  const badgeContent = document.createElement('span')
  badgeContent.textContent = 'BC'
  badgeContent.style.fontWeight = 'bold'
